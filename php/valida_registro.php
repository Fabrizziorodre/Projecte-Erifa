...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$hay_errores) { // si no hay errores
        ...
        if (mysqli_connect_errno()) {
            ...
            // Encriptar la contraseña y el número de tarjeta usando SHA-256
            $contrasenaHash = hash('sha256', $contra);
            $ntarjetaHash = hash('sha256', $ntarjeta);

            // Registrar usuario
            $query="insert into usuario (nombre_usuario, fecha_nacimiento, correo, contrasena, numero_tarjeta, direccion) values ('$nombre','$fechanacimiento','$correo','$contrasenaHash','$ntarjetaHash','$address');";
            mysqli_query($con, $query);
           
            // Obtener id
            $result = mysqli_query($con, "SELECT id_usuario, super_usuario, nombre_usuario FROM usuario where nombre_usuario='$nombre' and contrasena='$contrasenaHash';");
